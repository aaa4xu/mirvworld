<script lang="ts">
	...
	const { game }: { game: LiveGameStatus } = $props();

	const elapsed = $derived(Math.floor((Date.now() - game.startedAt) / 1000));
	const h = $derived(Math.floor(elapsed / 3600));
	const m = $derived(Math.floor((elapsed % 3600) / 60));
	const s = $derived(elapsed % 60);
</script>

<section style="background-image: url('/openfront/maps/{game.map.split(' ').join('')}Thumb.webp')">
	...
	<header>
		...
		<div class="status"><a href="https://openfront.io/join/{game.id}" target="_blank">Live</a></div>
	</header>

	<div class="info-container">
		<div class="info">
			<div>Map</div>
			<div class="info-value">{game.map}</div>
		</div>

		<div class="info">
			<div>Elapsed</div>
			<div class="info-value">
				{#if h > 0}{h.toString().padStart(2, '0')}:{/if}{m.toString().padStart(2, '0')}:{s
					.toString()
					.padStart(2, '0')}
			</div>
		</div>
		...
		<div class="info">
			<div>Players</div>
			<div class="info-value">
				{game.players}{#if game.maxPlayers}/{game.maxPlayers}{/if}
			</div>
		</div>
	</div>
</section>
